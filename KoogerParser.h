...
public:
    ...
    static constexpr int Setting = 2;
    static constexpr int Getting = 3; //aka Request
    static constexpr int Content = 4; //aka Data
    static constexpr int Action = 5; //aka Reserved
    static constexpr int Reaction = 6; //aka Reserved
    static constexpr uint8T CMD_RESP = 0x01;
    static constexpr uint8T CMD_CHART = 0x0A;
    static constexpr uint8T CMD_ARRAY = 0x0B;
    static constexpr uint8T CMD_YPR = 0x0C;
    static constexpr uint8T CMD_QUAT = 0x0D;
    static constexpr uint8T CMD_TEMP = 0x12;
    static constexpr uint8T CMD_DIAG = 0x13;
    static constexpr uint8T CMD_AGC = 0x14;
    static constexpr uint8T CMD_TRANSC = 0x15;
    static constexpr uint8T CMD_SND_SPD = 0x16;
    static constexpr uint8T CMD_UART = 0x17;
    static constexpr uint8T CMD_FLASH = 0x1D;
    static constexpr uint8T CMD_FORMAT = 0x1E;
    static constexpr uint8T CMD_REBOOT = 0x1F;
    static constexpr uint8T CMD_MARK = 0x20;
    static constexpr uint8T CMD_VERSION = 0x21;
...
private:
    ...
    std::function<void (const Resp &resp)> respCallback;
    std::function<void (const ChartDataMode &chartData)> chartDataCallback;
    std::function<void (const ChartRqstMode &rqstData)> chartRqstCallback;
    std::function<void (const YPR &ypr)> YPRCallback;
    std::function<void (const YPRattch &yprAttch)> YPRattchCallback;
    std::function<void (const Temp &temp)> tempCallback;
    std::function<void (const Array &array)> arrayCallback;
    std::function<void (const QUAT &quat)> quatCallback;
    std::function<void (const Diag &diag)> diagCallback;
    std::function<void (const AGC &agc)> agcCallback;
    std::function<void (const Transc &transc)> transcCallback;
    std::function<void (const Speed &speed)> speedCallback;
    std::function<void (const UART &uart)> uartCallback;
    std::function<void (const MARK &mark)> markCallback;
    std::function<void (const Version &version)> versionCallback;
};
